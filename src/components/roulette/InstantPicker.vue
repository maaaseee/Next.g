...
<template>
  <div class="flex flex-col items-center justify-center gap-7 py-4">
    ...
    <div
      class="relative w-full max-w-md p-5 rounded-lg border shadow-xl flex flex-col items-center justify-center min-h-60 overflow-hidden"
      :style="{
    ...
    >
      ...
      <div class="w-full flex items-center justify-between pb-2 mb-3 border-b text-xs" :style="{ borderColor: 'var(--app-border)' }">
        <div class="flex items-center gap-1.5 font-mono text-[11px]" :style="{ color: 'var(--app-text-muted)' }">
          ...
          <span>ESTADO: {{ isSelecting ? 'SELECCIONANDO...' : 'LISTO' }}</span>
        </div>
      ...
      <div v-if="previewGame" class="flex flex-col items-center text-center gap-2.5 z-10 w-full">
        <div
          ...
          :class="{ 'scale-105 border-(--app-primary)': isSelecting, 'border-white/10': !isSelecting }"
        >
          <img
        ...
        <h3 class="text-base font-bold tracking-tight max-w-70 truncate" :style="{ color: 'var(--app-text)' }">
          {{ previewGame.title }}
        </h3>
      ...
      <!-- Idle Ready Display -->
      <div v-else class="flex flex-col items-center justify-center text-center gap-2 z-10 py-3">
        <div
          class="w-11 h-11 rounded-md flex items-center justify-center border shadow-xs"
          :style="{
            backgroundColor: 'var(--app-surface-hover)',
        ...
        <p class="text-[11px] max-w-xs" :style="{ color: 'var(--app-text-muted)' }">
          Selecciona un juego de tu backlog de manera instantánea.
        </p>
      </div>
    </div>
    ...
    <div class="flex flex-col items-center gap-1.5">
      ...
      >
        ...
        <span>{{ isSelecting ? 'Eligiendo...' : 'Selección Rápida' }}</span>
      </button>
